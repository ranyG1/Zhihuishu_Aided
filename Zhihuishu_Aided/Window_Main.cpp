...
Window_Main::Window_Main(QPoint Start1_isd, QPoint End1_isd, QPoint Start2_isd, QPoint End2_isd, QPoint Start3_isd, QPoint End3_isd, QWidget *parent) : QWidget(parent)
{
	...
	connect(T2, SIGNAL(Touch(void)), this, SLOT(Judge(void)));//线程间连接
}
...
void Window_Main::ToClick(int mode)
{
	...
	case 2:
	{
		//选一个选项 选项位置大致对应的坐标为以弹题框左上角为原点，水平385，垂直200
		...
		Sleep(1000);
		//点击关闭 关闭按钮对应的坐标为以弹题框左上角为原点，水平385，垂直425
...
void Thread_ScreenAnalyze::run(void)
{
	while (flag_Thread_ScreenAnalyze)
	{
		...
		//TODO
		//未知原因，win10上截图黑屏 2018.10.10
		//已定位是chrome的原因 2018.11.26
		//查证是由chrome硬件加速引起，暂时关闭chrome硬件加速选项即可 2019.06.19
		...
		//简单粗暴，弹题区域截图判断RGB均>=200的像素点占总区域的百分比>=0.9即可认为弹题了
		int PixelSub = 0;
		...
		if (!IfQuestionShow)//如果弹题了，忽略检测到的视频结束标识
		{
			...
			//同样简单粗暴，二值化图像满足亮区占有率在69.640%(实验测得)+-3%区域内即可认为有暂停标志出现
...
void Window_Main::keyPressEvent(QKeyEvent *event)//内部按键检测
{
